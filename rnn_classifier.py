...
def ascii_onehot2word(onehot_array):
    #global name_datset
...
class name_Dataset(Dataset):
    def __init__(self):
        ...
        self.name_list = words2onehot(self.name_list)
        self.country_label_tensor = torch.LongTensor(self.country_label) #should be LongTensor

    def __getitem__(self, index):
        return self.name_list[index],self.country_label[index]
...
def collate_fn(data_tensor):
    seq_len_list = []
    X_tuple,label_tuple = zip(*data_tensor)
    for s in X_tuple:
        seq_len_list.append(s.shape[0])
    seq_len_tensor = torch.Tensor(seq_len_list)
    X_pad = pad_sequence(X_tuple)
    label_tensor = torch.LongTensor(label_tuple)
    return (X_pad,seq_len_tensor),label_tensor
